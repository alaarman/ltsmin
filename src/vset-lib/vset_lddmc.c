...
set_save(FILE* f, vset_t set)
{
    ...
    size_t meta = lddmc_serialize_add(set->meta);
...
set_load(FILE* f, vdom_t dom)
{
    ...
    if (fread(&meta, sizeof(size_t), 1, f) < 1) Abort("Invalid file format.");
    if (fread(&set->size, sizeof(int), 1, f) < 1) Abort("Invalid file format.");
    set->mdd = lddmc_serialize_get_reversed(mdd);
    set->meta = lddmc_serialize_get_reversed(meta);
...
{
    size_t vector_size;
    if (fread(&vector_size, sizeof(size_t), 1, f) < 1) Abort("Invalid file format.");
